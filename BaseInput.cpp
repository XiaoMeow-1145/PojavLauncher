    ...
    VR_ARRAY_COUNT(originOutCount) VRInputValueHandle_t* originsOut, uint32_t originOutCount)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		uint32_t len;
		XrResult res = xrPathToString(xr_instance, tmp[i], XR_MAX_PATH_LENGTH, &len, buff);
		if (res == XR_ERROR_PATH_INVALID) {
			OOVR_LOG_WARN("GetActionOrigins: skipping invalid path (XrPath=0x%lx)", tmp[i]);
			continue;
		}
